...
class GetStudentActivityCommand implements BackendCommand {
    ...
    async execute(): Promise<any> {
        ...
        if (!((await this.backendCommandReceiver.getActivityDetails(this.activityID)) === null)){
            //Testa se Estudante existe
            if (!((await this.backendCommandReceiver.getStudentAnalytics({"activityID":this.activityID,"InveniRAstdID":this.InveniRAstdID})) === null)){
				...
				return {
                    ...
                    "InveniRAstdID":this.InveniRAstdID,
                    "status":400,
                    "erro":"Erro! Estudante "+this.InveniRAstdID+" não cadastrado na Atividade "+this.activityID+"!",
                }
			}
		} else {
			return {
                ...
                "InveniRAstdID":this.InveniRAstdID,
                "status":400,
                "erro":"Erro! Atividade "+this.activityID+" não cadastrada!",
            }
		}
